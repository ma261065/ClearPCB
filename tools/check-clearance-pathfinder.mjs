// Comprehensive clearance check for the Pathfinder (negotiated-congestion)
// autorouter results. Mirrors tools/check-clearance-full.mjs but invokes
// `routeAllPathfinder` instead of `routeAll`.
//
...
console.log(`[pathfinder] board=${boardFile} traceWidth=${traceWidth} clearance=${clearance} viaDia=${viaDiameter}`);
...
for (const via of vias) {
    for (const pad of board.allObstaclePads) {
        const d = padPointDist(via.x, via.y, pad);
